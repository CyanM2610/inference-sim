...
func runWithObservation(c Config, factories PolicyFactories, observation *runObservation) (*Result, error) {
	...
	if c.EnginePhases != nil {
		...
		if c.HotPrefix != nil && c.HotPrefix.Benefit != nil {
			...
			if c.HotPrefix.ReclaimMode == "deficit_tail" {
				out.HotPrefixCostCoverage += "; deficit_capped_logarithmic_tail_options; extra_value_evaluations_measured_locally_not_native_calibrated"
			}
